...
**工具模板中的依赖声明**（见 4.4 节）会直接驱动此检查逻辑，无需硬编码。
...
将工具定义与其运行依赖一并模板化，方便用户添加自定义工具，也使官方内置工具可作为参考实现。

**模板文件格式**（`.localcheck/plugins/builtin/coverage.toml`）：
...
report_out  = "report/quality/coverage.md"

# 运行依赖声明
[[tool.deps]]
bin     = "cargo-tarpaulin"          # 检测的可执行文件名
install = "cargo install cargo-tarpaulin"   # 有方案时提供
auto_install = true                  # 是否允许自动安装

[[tool.deps]]
bin     = "cargo"
install = ""                         # 空字符串 = 无标准安装命令，仅给建议
hint    = "请先安装 Rust 工具链：https://rustup.rs"
```

- `rust-checker plugin add <name>` 从插件注册表安装社区模板到 `plugins/custom/`
- 内置工具本身作为官方插件的参考实现，放在 `plugins/builtin/`
- 自定义工具只需新建一个 `.toml` 文件，无需修改主配置
...
| P1 | 工具模板化（含依赖声明字段） | 插件机制基础 + 用户扩展入口 |
| P2 | `diff` 命令 | 趋势追踪 |
| P2 | 插件注册表 / `plugin add` | 社区生态 |
| P3 | Watch 模式 | 开发体验提升 |
