...
- [x] HTML 格式提供带样式的 Dashboard 页面（card 布局，含统计栏）
...
- [x] 提供插件贡献指南（`docs/plugin-guide.md`，含 plugin.toml 规范 + 贡献流程）
...
- [x] 明确区分 Windows / Linux / macOS 的系统依赖安装提示（`dependency_check.rs` 新增 `SystemInstallHint`）
...
- [x] 跨平台 CI 矩阵（ubuntu-latest / macos-latest / windows-latest）
- [x] `diff` 命令集成测试（Phase 3）——已补充多场景测试（diff latest、range、last、单条目）
...
- [x] 插件开发指南（`docs/plugin-guide.md`，含 plugin.toml 规范 + 贡献流程）
...
| `diff` 命令集成测试 | `tests/integration.rs` 补充 diff latest/range/last/单条目测试 | ✅ |

### 待完成

| 建议 | 说明 |
|------|------|
| HTML 折线趋势图 | 需引入前端图表库（如 Chart.js），工程量较大，留待后续版本 |
